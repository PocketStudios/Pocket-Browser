...
function nextPage() {
    switch(page) {
        ...
        case 4:
            ...
            document.getElementById("content").innerHTML = "Found any bug? Report it to us!<br><br><a href='emailto:support@pocket-inc.com'>support@pocket-inc.ml</a>"
...
function errorData(state) {
    ...
    if (state === 0) {
        fs.writeFileSync(require("path").join(dataPath, "/errors.pocket"),"false")
    } else if (state === 1) {
        fs.writeFileSync(require("path").join(dataPath, "/errors.pocket"),"true")
    }
    nextPage();
}
async function searchEngine() {
    ...
    if (engine == "Google") {
        await fs.writeFileSync(require("path").join(dataPath, "/search.pocket"),"https://google.com/search?q=%s");
    } else if (engine == "DuckDuckGo") {
        await fs.writeFileSync(require("path").join(dataPath, "/search.pocket"),"https://duckduckgo.com/?q=%s");
    } else if (engine == "Bing") {
        await fs.writeFileSync(require("path").join(dataPath, "/search.pocket"),"https://bing.com/search?q=%s");
    }
    document.getElementById('changed').innerHTML = "Changed to " + engine;
}

function changeMode(mode) {
    fs.writeFile(require("path").join(dataPath, '/security.pocket'), mode, function (err) {
        if (err) {
            throw err;
        }
    });
